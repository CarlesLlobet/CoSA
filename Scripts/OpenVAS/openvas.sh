        ...
        then
            ...
            if [ "$STATE" == "Blocked" ]; then
                ...
                curl -k -X GET http://localhost:8080/API/OpenVAS/kill/$id/
                    ...
                    percentage=100
                    curl -k -X PUT -d percentage=$percentage http://localhost:8080/API/OpenVAS/set_percentage/$id/
                    curl -k -X PUT -d state=Finished http://localhost:8080/API/OpenVAS/set_state/$id/
            fi
            rm $TMP/.running"$COUNTER"
            let N-=1
        else
...
then
    ...
    read next <<< `curl -k -X GET http://localhost:8080/API/OpenVAS/get_next/|sed 's/\"//g'`
	...
	if [[ $id ]]; then
        ...
        if [[ $ID_TARGET ]]; then
            ...
            ID_TARGET=`echo $CREATE_TARGET | awk '{print $6}' FS=" " | awk '{print $2}' FS="\""`
            echo "Target created"
        fi
        if [[ $ID_TARGET ]]; then
            echo "Target ID:" $ID_TARGET
        fi
        ...
        if [[ $name ]]; then
            ...
            read -d "\n" ID_TASK aux <<< `$BINDIR/omp -u "$USER" -w "$PASSWD" -G | grep $name | tr " " "\n"`
        else
            #Create the scan
            ...
            ID_TASK=`echo $CREATE_TASK | awk '{print $6}' FS=" " | awk '{print $2}' FS="\""`
            echo "Task created"
        fi

        if [[ $ID_TASK ]]; then
            echo "Task ID:" $ID_TASK
        fi
        ...
        curl -k -X PUT -d state=Running http://localhost:8080/API/OpenVAS/set_state/$id/
        curl -k -X PUT -d report=$ID_TASK_STARTED http://localhost:8080/API/OpenVAS/set_report/$id/
